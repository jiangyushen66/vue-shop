    ...
    <el-card>
      ...
      <tree-table :data="cateList" :columns="columns" :selection-type="false" :expand-type="false" show-index border class="tree-table">
        ...
        <template slot="operate" slot-scope="scope">
          <el-button size="mini" class="el-icon-edit" type="primary" @click="showEditDialog(scope.row.cat_id)">编辑</el-button>
          <el-button size="mini" class="el-icon-delete" type="danger" @click="removeCateById(scope.row.cat_id)">删除</el-button>
        </template>
      </tree-table>
...
export default {
  data() {
    return {
      ...
      addForm: {
        cat_name: '', //分类的名称
        cat_pid: 0, //父级分类的id，默认为一级分类
        cat_level: 0, //分类层级，默认的为一级分类
      },
      addFormRules: {
      ...
      cascaderProps: {
        ...
        children: 'children',
        expandTrigger: 'hover',
        checkStrictly: true, //级联选择器中父子节点不相互关联
      },
      //级联选择器选中的id数组
      selectedKeys: [],
  ...
  methods: {
    ...
    async showEditDialog(cateId) {
      ...
      if (res.meta.status !== 200) {
        return this.$message.error('查询分类失败失败')
      }
