        ...
        <el-table-column label="明细" type="expand">
          <template slot-scope="scope">
            ...
            <el-table :data="scope.row.detail" border stripe>
              ...
              <el-table-column label="商品的数量" prop="goods_number" width="100px"></el-table-column>
              <el-table-column label="商品的价格（元）" prop="goods_price" width="100px"></el-table-column>
  ...
  methods: {
    ...
    editAddress() {
      this.$refs.addressFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.put(`orders/${this.order_id}/address`, { consignee_addr: this.addressForm.address1.join(',') + ',' + this.addressForm.address2 })
        if (res.meta.status !== 200) {
