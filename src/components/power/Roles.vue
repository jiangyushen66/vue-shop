<template>
  <div>
    ...
    <el-card>
      ...
      <!-- 角色列表 -->
      <el-table :data="rolesList" border stipe>
        <!-- 展开列 -->
        <el-table-column label="明细" type="expand">
          <template slot-scope="scope">
            <!-- 渲染一级权限 -->
            <!-- <el-row v-for="(item1, i1) in scope.row.children" :key="item1.id" :class="{bdbottom:true}"> -->
    ...
    removeRightById(role, rightId) {
      this.$confirm('确定要删除该权限吗？', '提示', {
        ...
        .then(async () => {
          ...
          // this.getRolesList();//不建议重新获取所有角色列表，会导致页面重新刷新
          role.children = res.data //重新设置当前角色下最新的角色数据
